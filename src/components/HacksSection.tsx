...
const HacksSection: React.FC<HacksSectionProps> = ({ hacks, completedHacks, onActivateClick, onAmplifyClick, playUIClick }) => {
    ...
    // Latest ref pattern to stabilize callbacks without stale closures
    const onActivateRef = useRef(onActivateClick);
    const onAmplifyRef = useRef(onAmplifyClick);
    const playUIClickRef = useRef(playUIClick);

    useEffect(() => {
        onActivateRef.current = onActivateClick;
        onAmplifyRef.current = onAmplifyClick;
        playUIClickRef.current = playUIClick;
    });

    const handleActivate = useCallback((id: number) => {
        playUIClickRef.current();
        onActivateRef.current(id);
    }, []);

    const handleAmplify = useCallback((id: number) => {
        playUIClickRef.current();
        onAmplifyRef.current(id);
    }, []);
