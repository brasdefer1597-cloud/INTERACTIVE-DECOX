...
}> = React.memo(({ hack, isCompleted, onActivate, onAmplify }) => {
...
const HacksSection: React.FC<HacksSectionProps> = React.memo(({ hacks, completedHacks, onActivateClick, onAmplifyClick, playUIClick }) => {
    ...
    // Memoize archetypes to prevent recalculation
    const archetypes = React.useMemo(() => {
        return Array.from(new Set(hacks.map(h => h.archetype)));
    }, [hacks]);

    // Find active combos and memoize
    const activeCombos = React.useMemo(() => {
        return Object.entries(COMBOS).filter(([key]) => {
            const ids = key.split('-').map(Number);
            return ids.every(id => completedHacks.has(id));
        });
    }, [completedHacks]);

    // Stable handlers for HackCard components
