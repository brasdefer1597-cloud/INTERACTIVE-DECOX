...
// Wrapped HackCard in React.memo to prevent unnecessary re-renders of list items when parent state changes.
// Used useCallback to create stable references for the specific item's handlers.
const HackCard: React.FC<{ hack: Hack; isCompleted: boolean; onActivate: (id: number) => void; onAmplify: (id: number) => void; }> = React.memo(({ hack, isCompleted, onActivate, onAmplify }) => {
    const handleActivate = React.useCallback(() => onActivate(hack.id), [onActivate, hack.id]);
    const handleAmplify = React.useCallback(() => onAmplify(hack.id), [onAmplify, hack.id]);
...
// ⚡ Bolt Performance Optimization:
// Wrapped HacksSection in React.memo to avoid re-rendering the entire list unless relevant props change.
const HacksSection: React.FC<HacksSectionProps> = React.memo(({ hacks, completedHacks, onActivateClick, onAmplifyClick, playUIClick }) => {
    // ⚡ Bolt Performance Optimization: Memoized derived data computations
    const archetypes = React.useMemo(() => Array.from(new Set(hacks.map(h => h.archetype))), [hacks]);

    // Find active combos
    const activeCombos = React.useMemo(() => Object.entries(COMBOS).filter(([key]) => {
        const ids = key.split('-').map(Number);
        return ids.every(id => completedHacks.has(id));
    }), [completedHacks]);

    // ⚡ Bolt Performance Optimization:
    // Stabilized wrapper handlers that pass the id, instead of generating inline closures for each item.
