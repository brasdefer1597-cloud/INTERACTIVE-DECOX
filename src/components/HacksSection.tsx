...
const HacksSection: React.FC<HacksSectionProps> = React.memo(({ hacks, completedHacks, onActivateClick, onAmplifyClick, playUIClick }) => {
    ...
    // Create a stable ref for external callbacks to prevent closures going stale
    // while keeping our handler references stable for HackCard's React.memo
    const callbacksRef = React.useRef({ onActivateClick, onAmplifyClick, playUIClick });
    React.useEffect(() => {
        callbacksRef.current = { onActivateClick, onAmplifyClick, playUIClick };
    }, [onActivateClick, onAmplifyClick, playUIClick]);
