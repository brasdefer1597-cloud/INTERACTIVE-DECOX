...
interface HackDetailsProps {
    hack: Hack;
    poder?: PoderDeSherezade;
    inputs: Record<string, string>;
    onInputChange: (field: string, value: string) => void;
}

const HackDetails: React.FC<HackDetailsProps> = ({ hack, poder, inputs, onInputChange }) => {
    return (
                                        ...
                                        <div className="p-4 space-y-8 bg-black/30 rounded-lg">
                                            ...
                                            <div className="text-center">
                                                ...
                                                <div className="fractal-node central-node p-4 mt-2">
                                                    ...
                                                    <textarea
                                                        rows={3}
                                                        value={inputs['fractal_central'] || ''}
                                                        onChange={(e) => onInputChange('fractal_central', e.target.value)}
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
                                                    <div className="fractal-node context-node p-4 flex flex-col">
                                                        ...
                                                        <textarea rows={4} placeholder="Ej: Caída de imperios por falta de comunicación..." className="flex-grow"
                                                            value={inputs['fractal_historico'] || ''}
                                                            onChange={(e) => onInputChange('fractal_historico', e.target.value)}
                                                        />
                                                    </div>
                                                    <div className="fractal-node context-node p-4 flex flex-col">
                                                        ...
                                                        <textarea rows={4} placeholder="Ej: Empresas que quiebran por ignorar al mercado..." className="flex-grow"
                                                            value={inputs['fractal_industrial'] || ''}
                                                            onChange={(e) => onInputChange('fractal_industrial', e.target.value)}
                                                        />
                                                    </div>
                                                    <div className="fractal-node context-node p-4 flex flex-col">
                                                        ...
                                                        <textarea rows={4} placeholder="Ej: Especies que se extinguen por no adaptarse..." className="flex-grow"
                                                            value={inputs['fractal_biologico'] || ''}
                                                            onChange={(e) => onInputChange('fractal_biologico', e.target.value)}
                                                        />
                                                    </div>
                                                </div>
                                            </div>
                    ...
                    <div className="space-y-4 bg-gray-900 p-4 rounded-lg">
                        {poder.plantilla.map(item => (
                            <div key={item.aspecto}>
                                ...
                                <textarea
                                    rows={3}
                                    value={inputs[item.aspecto] || ''}
                                    onChange={(e) => onInputChange(item.aspecto, e.target.value)}
